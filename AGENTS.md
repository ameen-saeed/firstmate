...
A remote secondmate steer rides the same durable-inbox model through the remote transport, and an unconfirmed remote delivery is safe to re-run because the remote enqueue deduplicates the same request (`bin/fm-send.sh` header).
