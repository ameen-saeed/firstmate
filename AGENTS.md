...
config/pi-supervision-branch  Pi-only explicit autonomy grant for the in-process supervision branch that absorbs ordinary actionable fleet wakes on a Pi primary; LOCAL, gitignored, not inherited; exact "on" = enabled, absent or anything else = today's wake-to-main behavior; see docs/configuration.md "Pi supervision branch" and docs/pi-supervision-branch.md
